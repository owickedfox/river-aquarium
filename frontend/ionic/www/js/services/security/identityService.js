...
angular.module('RiverAquarium.Services.Security')
    .factory('$IdentityService', function ($q, $http, $timeout) {

        // $IdentityService is a service that tracks the user's identity. 
        ...
        return {
            ...
            isInRole: function (role) {
                if (!_isAuthenticated || !_identity.roles) {
                    return false;
                }
            ...
            authenticate: function (identity) {
                _identity = identity;
                _isAuthenticated = identity !== null;

                // for this demo, we'll store the identity in localStorage. For you, it could be a cookie, sessionStorage, whatever
                if (identity) {
                    localStorage.setItem('ra.identity', angular.toJson(identity));
                } else {
                    localStorage.removeItem('ra.identity');
                }
            },
            checkIdentity: function (force) {
                ...
                var self = this;
                $timeout(function () {
                    _identity = angular.fromJson(localStorage.getItem('ra.identity'));
